...
public class Suord : MonoBehaviour {

    public static Suord instance;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "Foe")
        {
            ...
            if (player.heavyWeaponSelected) {
                collision.gameObject.GetComponent<Damageable>().GetDamaged(player.heavyDamage, bodyCollider);
            }
            else
            {
                collision.gameObject.GetComponent<Damageable>().GetDamaged(player.lightDamage, bodyCollider);
            }
        }
    }
}
